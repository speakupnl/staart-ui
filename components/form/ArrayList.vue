<template>
  <div class="form-group">
    ...
    <table v-if="list.length" class="table" style="margin-bottom: 0">
      <tbody>
    ...
    <button
      v-if="!disabled"
      :class="
        `button button--type-icon ${list.length ? '' : 'text text--mt-1'}`
      "
      type="button"
      @click.prevent="add"
    >
...
export default class ArrayList extends Vue {
  @Prop() value;
  @Prop() label;
  @Prop() required;
  @Prop() placeholder;
  ...
  private mounted() {
    if (this.value) this.list = [...this.value];
  }
